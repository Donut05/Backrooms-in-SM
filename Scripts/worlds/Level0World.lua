...
Level0World.terrainScript = "$GAME_DATA/Scripts/terrain/terrain_flat.lua" --SMBACKROOMS For faster loading times and no annoying ship ambience
Level0World.groundMaterialSet = "$GAME_DATA/Terrain/Materials/gnd_standard_materialset.json"
Level0World.enableSurface = true
Level0World.enableAssets = true
Level0World.enableClutter = true
...
Level0World.chunkSize = Level0World.levelData[4].x

local seed = "113489167498141848093719847194717412904719824587981759178"
--local bird_level

function  Level0World.sv_gotoLevel( self, args ) -- SMBACKROOMS
	sm.character.createCharacter(args.player, self.world, sm.vec3.zero())
end
...
function Level0World.server_onCreate( self )
	...
	print( "Level0World.server_onCreate" )
...
function Level0World.client_onCreate( self )
	...
	print( "Level0World.client_onCreate" )

	self.ambienceEffect = sm.effect.createEffect( "ChallengeAmbience" )
...
function Level0World.client_onDestroy( self )
...
function Level0World.server_onRefresh( self )
end

function Level0World.server_onFixedUpdate( self )
	BaseWorld.server_onFixedUpdate( self )

	g_unitManager:sv_onWorldFixedUpdate( self )
	if sm.player.getAllPlayers()[1].character then
		local currentChunk = {x=math.floor((sm.player.getAllPlayers()[1].character.worldPosition/self.chunkSize).x),y=math.floor((sm.player.getAllPlayers()[1].character.worldPosition/self.chunkSize).y)}
		local chunkStart = {x=currentChunk.x * self.chunkSize,y=currentChunk.y*self.chunkSize}
		local currentSeedIndex = tonumber(((currentChunk.x+currentChunk.y)%#seed)+1)
		print(string.sub(seed,currentSeedIndex,currentSeedIndex))
	end
end

function Level0World.client_onFixedUpdate( self )
...
function Level0World.cl_n_unitMsg( self, msg )
	g_unitManager[msg.fn]( g_unitManager, msg )
end

function Level0World.sv_loadShipOrStationOnCell( self, x, y )
...
function Level0World.sv_loadHideoutOnCell( self, x, y )
...
function Level0World.sv_loadPathNodesOnCell( self, x, y )
...
function Level0World.sv_reloadPathNodesOnCell( self, x, y )
...
function Level0World.sv_loadSpawnersOnCell( self, x, y )
...
function Level0World.sv_reloadSpawnersOnCell( self, x, y )
	local nodes = sm.cell.getNodesByTag( x, y, "PLAYER_SPAWN" )
	g_respawnManager:sv_setLatestSpawners( nodes )
end

function Level0World.sv_spawnNewCharacter( self, params )
...
function Level0World.client_onCellLoaded( self, x, y )
...
function Level0World.server_onCellUnloaded( self, x, y )
	BaseWorld.server_onCellUnloaded( self, x, y )
	--print( "Level0World - cell ("..x..","..y..") unloaded" )
	g_unitManager:sv_onWorldCellUnloaded( self, x, y )
end

function Level0World.client_onCellUnloaded( self, x, y )
	BaseWorld.client_onCellUnloaded( self, x, y )
	--print( "Level0World - client cell ("..x..","..y..") unloaded" )
end
...
function Level0World.sv_ambush( self, params )
...
function Level0World.sv_e_spawnRaiders( self, params )
...
function Level0World.sv_e_spawnTempUnitsOnCell( self, params )
